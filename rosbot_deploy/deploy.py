...
def create_deployment(robomaker, fleet_arn, app_arn, app_ver):
    package_name = 'hello_world_robot'
    launch_file = 'rotate.launch.py'
...
def start_deployment(user_fleet_name, user_robot_name, s3bucket):
    ...
    app_key_prefix = 'helloworldsample/robot_ws/bundle'
    print('Clean fleet')
    ...
    if rosbot_arn:
        ...
        return None
    application_arn = get_robot_application_arn(robomaker_client, 'HelloWorldSample')
    if application_arn:
        add_armhf_to_app(robomaker_client, application_arn, s3bucket, app_key_prefix)
    else:
        application_arn = create_robot_app(robomaker_client, 'HelloWorldSample', s3bucket, app_key_prefix)
    app_version = get_current_application_version(robomaker_client, application_arn)
    create_deployment(robomaker_client, fleet_arn, application_arn, app_version)
